...
public class Enemy {
	...
	public void update(float deltaTimeElapsedMs) {
		...
		centerX += speedX;
		speedX = Assets.bg1.getSpeedX() * 5 + movementSpeed;
	...
	public void follow() {
		if (centerX < -95 || centerX > 810) {
			movementSpeed = 0;
		} else if (Math.abs(Assets.hero.getCenterX() - centerX) < 5) {
			movementSpeed = 0;
		} else {
			if (Assets.hero.getCenterX() >= centerX) {
				movementSpeed = 1;
			} else
				movementSpeed = -1;
		}
